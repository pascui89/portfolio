...
import { ChakraProvider, chakra, Box, Fade, Stack, Progress, Divider, SimpleGrid, Card, CardHeader, 
  Heading, CardBody, Text, Flex, Avatar, IconButton } from '@chakra-ui/react';
...
const renderCompanyInfo = (company: string, years: number[], textCompany: string) => 
    <Card maxW='2lg'>
      <CardHeader>
        <Flex>
...
const renderProcess = (technology: string, percent: number) => {
    ...
    return (  
      <Box display={'flex'}>
        <chakra.p flex={1} textAlign={'left'}>{technology}</chakra.p><Progress flex={1} mt={2} colorScheme='green' size='sm' value={currentPercent} />
      </Box>
    );
}
        ...
        <Box width="75%">
          <Fade in={isLoaded} transition={{ enter: { duration: 0.5 } }}>
            <Box as="p" textAlign="justify" fontSize="lg" lineHeight="tall" marginBottom={4}>
              ¡Bienvenido/a a mi página de portfolio!
            </Box>
            <Box as="p" textAlign="justify" fontSize="lg" lineHeight="tall" marginBottom={4}>
            ...
            <SimpleGrid spacing={4} templateColumns='repeat(auto-fill, minmax(200px, 1fr))'>
              {experience.map((e) => renderCompanyInfo(e.job, e.years, e.text))}
            </SimpleGrid>
            <Box as="p" textAlign="justify" fontSize="lg" lineHeight="tall" marginBottom={4}>
